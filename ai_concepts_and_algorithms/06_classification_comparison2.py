...
# Assignment #06: Comparison of Classification using 8 algorithms
# Decision Tree vs. K-nearest neighbors vs. Artificial Neural Network vs. Logistic Regression
# vs. Support Vector Machine vs. Bagged Trees vs. Patched Trees vs Random Forests
...
def plot_data_as_boxplot_and_distplot(data, title, save=False, location=""):
    """
    Creates a boxplot and distplot based on <data> with <title>
    and saves the file in <location> when <save> is True.
    Otherwise, the plots are created and shown at the end of the execution.
    :param data: Numpy array
    :param title: string, used as title in the plot
    :param save: boolean, whether to save plot to file
    :param location: string, folder where to save the plots to
    """
    plt.figure(f"{title}_boxplot")
    plt.boxplot(data)
    plt.title(title)
    if save:
        plt.savefig(location + f"/{title.replace(':', '').replace(' ', '_')}_boxplot.png")
    plt.figure(f"{title}_distplot")
    distplot(data)
    plt.title(title)
    if save:
        plt.savefig(location + f"/{title.replace(':', '').replace(' ', '_')}_density.png")


def create_data_set(number_of_dataset, number_of_samples, save=False, location=""):
    """
    Create a random dataset using make_classification() and scale the data.
    Prepare 2 plots per dataset (and its scaled counterpart) and save them if <save> is True.
    ...
    :param location: str, location where to save plots
    :return features, scaled features, target of dataset
    """
    number_of_dataset += 1  # offset to start with 1
    X, y = datasets.make_classification(n_samples=number_of_samples)
    plot_data_as_boxplot_and_distplot(X, f"Original dataset: {number_of_dataset}", save=save, location=f"{location}")

    scaled_X = normalize(X)
    plot_data_as_boxplot_and_distplot(scaled_X, f"Scaled dataset: {number_of_dataset}", save=save, location=f"{location}")
...
def perform_one_iteration(X, y, k, ann_hyperparameters):
    ...
    * Patched Decision Tree Classification
    * Random Forest
    ...
    # Support Vector Machine
    svm = classify_with_support_vector_machine(X_train, y_train, X_test, y_test)

    # Bagged Trees
    bagged = classify_with_bagged_trees(X_train, y_train, X_test, y_test)

    # Patched Trees
    patched = classify_with_patched_trees(X_train, y_train, X_test, y_test)

    # Random Forest
    rf = classify_with_random_forests(X_train, y_train, X_test, y_test)

    return decision_tree, knn, ann, log_reg, svm, bagged, patched, rf
...
def update_results(result, iteration, accuracies, scaled=False):
    ...
    if scaled:
        data_type = "scaled"
    decision_tree, knn, ann, log_reg, svm, bagged, patched, rf = accuracies
    ...
    result[iteration][data_type]["svm"].append(svm)
    result[iteration][data_type]["bagged"].append(bagged)
    result[iteration][data_type]["patched"].append(patched)
    result[iteration][data_type]["forest"].append(rf)
    return result


def plot_accuracies(accuracy_for_all_runs, save=False, location=""):
    """
    Plot mean accuracies and their standard deviation per dataset in a barplot.
    :param accuracy_for_all_runs: dictionary, calculated mean accuracies
    :param save: bool, whether to save plots to file
    :param location: location where to save the files to
    """
    for dataset, methods in accuracy_for_all_runs.items():
        for method, algorithms in methods.items():  # method is either original or scaled
            title = f"Dataset {dataset + 1} - {method}"
            plt.figure(title)
            plt.title(title)
            plt.ylim(0, 1.1)
            for index, algorithm in enumerate(algorithms):  # the 8 used algorithms
                algorithm_list = list(algorithms.keys())
                plt.bar(algorithm_list[index], accuracy_for_all_runs[dataset][method][algorithm][0],
                        yerr=accuracy_for_all_runs[dataset][method][algorithm][1])
            if save:
                plt.savefig(f"{location}/{dataset + 1}_{method}_bar.png")


def calculate_mean_accuracy(result):
    """
    Calculate the mean accuracy and its standard deviation for all Cross validation runs.
    ...
    for dataset in result:
        ...
        for datatype in result[dataset]:
            ...
            for algorithm in result[dataset][datatype]:
                ...
                rounded_mean = round(np.mean(data_array), 2)
                rounded_std = round(np.std(data_array), 2)
                results[dataset][datatype][algorithm] = (rounded_mean, rounded_std)
    return results
...
def pretty_print_results(results, n, k):
    ...
    for result in results:
        ...
        print(f"{'Algorithm'} | {'Original Data'} | {'Standard Dev.'} | {'Scaled Data'} | {'Standard Dev.'}")
        for algorithm in original_data:
            print(
                f"{algorithm: ^9} | {original_data[algorithm][0]: ^13} | {original_data[algorithm][1]: ^13} |  {scaled_data[algorithm][0]: ^9} | {scaled_data[algorithm][1]: ^13}")
        print(f"{'':_^70}")
...
def classify_with_support_vector_machine(X_train, y_train, X_test, y_test):
    """
    Perform classification using sklearn's Support Vector Classifier
    :param X_train: training features of the dataset
    :param y_train: training target of the dataset
    :param X_test: test features of the dataset
    :param y_test: test target of the dataset
    :return: calculated accuracy
    """
    classification = SVC().fit(X_train, y_train)
    predict = classification.predict(X_test)
    accuracy = calculate_accuracy(predict, y_test)
    return accuracy


def classify_with_bagged_trees(X_train, y_train, X_test, y_test):
    """
    Perform classification using sklearn's BaggingClassifier with bagging
    :param X_train: training features of the dataset
    :param y_train: training target of the dataset
    :param X_test: test features of the dataset
    :param y_test: test target of the dataset
    :return: calculated accuracy
    """
    classification = BaggingClassifier(bootstrap=True, bootstrap_features=False).fit(X_train, y_train)
    predict = classification.predict(X_test)
    accuracy = calculate_accuracy(predict, y_test)
    return accuracy


def classify_with_patched_trees(X_train, y_train, X_test, y_test):
    """
    Perform classification using sklearn's BaggingClassifier with patching
    :param X_train: training features of the dataset
    :param y_train: training target of the dataset
    :param X_test: test features of the dataset
    :param y_test: test target of the dataset
    :return: calculated accuracy
    """
    classification = BaggingClassifier(bootstrap=True, bootstrap_features=True).fit(X_train, y_train)
    predict = classification.predict(X_test)
    accuracy = calculate_accuracy(predict, y_test)
    return accuracy


def classify_with_random_forests(X_train, y_train, X_test, y_test):
    """
    Perform classification using sklearn's RandomForestClassifier
    :param X_train: training features of the dataset
    :param y_train: training target of the dataset
    :param X_test: test features of the dataset
    :param y_test: test target of the dataset
    :return: calculated accuracy
    """
    classification = RandomForestClassifier().fit(X_train, y_train)
    predict = classification.predict(X_test)
    accuracy = calculate_accuracy(predict, y_test)
    return accuracy
...
def ml_workflow(number_of_datasets=4, number_of_samples=1000, k_fold=10, save_plots=False, location=""):
    """
    Perform Machine Learning Workflow
    1. Create (or use) a dataset
    2. Scaling -> Normalization
    3. Hyperparameter tuning (in this case performed for knn and ann)
    4. K-fold Cross Validation

    Performed classification for <number_of_datasets> datasets, <k_fold> cross validations and 8 algorithms:

    :param number_of_datasets: int, amount of datasets to analyze
    :param number_of_samples: int, amount of samples per dataset
    :param k_fold: int, amount of cross validations
    :param save_plots: boolean, save plots to disk
    :param location: string, where to save images to
    :return: dictionary, mean accuracy and its std for all datasets and each algorithm used
    """
    result = {}
    for i in range(number_of_datasets):
        print(f"Analysis of dataset {i + 1}")
        original_X, scaled_X, y = create_data_set(i, number_of_samples=number_of_samples, save=save_plots,
                                                  location=location)
    ...
    plot_accuracies(mean_accuracy, save=save_plots, location=location)
...
mean_accuracies = ml_workflow(classification_datasets, 1000, k_fold_cross_validation, save_plots=True,
                              location="assignment_06")
